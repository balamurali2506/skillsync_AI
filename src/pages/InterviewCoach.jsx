...
import { Mic, MicOff, Volume2, Repeat, Pause, Play, PhoneOff, Keyboard, ChevronRight, Sparkles, Loader2, CheckCircle2, Lightbulb, Bot } from 'lucide-react';
...
export default function InterviewCoach() {
  ...
  // ── Setup state ──
  const [mode, setMode] = useState('setup'); // setup | mic | live | report
  ...
  const [liveStage, setLiveStage] = useState('starting'); // ai-speaking | waiting | listening | processing
  ...
  // ── Mic check ──
    ...
    try {
      ...
      an.fftSize = 512;  // More samples = smoother reading
      an.smoothingTimeConstant = 0.7;  // Smoother bar
      src.connect(an);
      
      const data = new Uint8Array(an.frequencyBinCount);
      const loop = () => {
        an.getByteTimeDomainData(data);
        // RMS calculation with higher sensitivity multiplier
        const rms = Math.sqrt(data.reduce((s, v) => s + ((v - 128) / 128) ** 2, 0) / data.length);
        // Boosted sensitivity: multiply by 5 instead of 3
  ...
  const startInterview = async () => {
    ...
    try {
      ...
      if (data.resumeUsed) toast.success(`Using resume: ${data.resumeUsed.fileName}`);
      askQuestion(data.question, 1, true);
    } catch {
  ...
  const askQuestion = (q, index, opening = false) => {
    ...
    speak(opening ? `Hello! I'm your AI interviewer. This is a simulated interview. Feel free to ask me to repeat any question. Let's begin. ${q.speakText || q.question}` : (q.speakText || q.question), {
      ...
      onEnd: () => {
        ...
        if (!typedMode) setTimeout(() => !pausedRef.current && startListening(), 700);
      },
    });
  };
  ...
  const startListening = () => {
    stopSpeaking();
    if (!sttSupported) { setTypedMode(true); setLiveStage('waiting'); return; }
    ...
    const listener = createListener({
      ...
      onError: (err) => {
        if (err === 'not-allowed') { toast.error('Microphone denied — switching to typing mode.'); setTypedMode(true); setLiveStage('waiting'); }
      },
    });
  ...
  const submitAnswer = async (text) => {
    const answerText = (text || (typedMode ? typedAnswer : transcript)).trim();
    if (!answerText) { toast.error('No answer captured — speak or type your answer.'); setLiveStage('waiting'); return; }
    listenerRef.current?.stop();
    stopSpeaking();
    setLiveStage('processing');
    const duration = answerStartRef.current ? Math.round((Date.now() - answerStartRef.current) / 1000) : 0;
    const fillerWords = FILLERS.filter((f) => new RegExp(`\\b${f.replace(' ', '\\s')}\\b`, 'i').test(answerText));

    try {
      const { data } = await api.post(`/interviews/${interviewId}/answer`, { answer: answerText, answerDuration: duration, fillerWords });
      if (data.done) {
        await finishInterview();
      } else {
        askQuestion(data.next, qIndex + 1);
      }
    } catch {
  ...
  const finishInterview = async () => {
    ...
    try {
      ...
      setReport(data);
      api.get(`/interviews/${interviewId}/report`).then(({ data: full }) => setFullReview(full)).catch(() => {});
      setMode('report');
    } catch {
  ...
  const statusText = {
    ...
    waiting: typedMode ? 'Type your answer below' : 'Listening standby — speak or tap the mic',
  ...
  if (mode === 'setup') return (
    <div className="mx-auto max-w-4xl space-y-6">
      <header>
        ...
        <p className="mt-2 text-sm sm:text-body text-neutral-500">A realistic voice interview that adapts to your answers — powered by your resume.</p>
      </header>
      ...
      <div className="rounded-3xl bg-white p-6 sm:p-8 shadow-card space-y-5">
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <label className="block">
            <span className="text-micro font-semibold uppercase tracking-wider text-neutral-500">Questions</span>
            <select value={config.questionCount} onChange={(e) => setConfig({ ...config, questionCount: Number(e.target.value) })}
              className="mt-1.5 w-full rounded-xl border border-neutral-200 bg-neutral-50 px-4 py-3 text-body">
              {[4, 6, 8, 10, 12].map((d) => <option key={d} value={d}>{d}</option>)}
            </select>
          </label>
        </div>
        ...
        <div className="rounded-2xl border border-brand-200 bg-gradient-brand-soft p-4">
          {resumeInfo ? (
            <>
              <p className="text-caption font-bold uppercase tracking-wider text-brand-700">
                Resume detected: {resumeInfo.fileName}
              </p>
            ...
            <p className="text-caption text-brand-800">No resume uploaded yet — questions will be role-based. Upload one in the Resume Analyzer for personalized questions.</p>
          )}
        </div>
  ...
  if (mode === 'report' && report) return (
    <div className="mx-auto max-w-4xl space-y-6">
      <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="rounded-3xl bg-neutral-950 p-8 sm:p-10 text-center text-white shadow-glow">
        ...
        <p className="mt-6 font-display text-display-xl font-bold text-brand-400"><CountUp value={report.overallScore} /></p>
      ...
      <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
        ...
        <div className="space-y-6">
          ...
          <div className="rounded-3xl bg-white p-6 shadow-card">
            <h3 className="text-title font-semibold text-amber-700">Improve</h3>
            <ul className="mt-3 space-y-2">{(report.weaknesses || []).map((s, i) => <li key={i} className="flex gap-2 text-body text-neutral-700"><Lightbulb size={16} className="mt-0.5 shrink-0 text-amber-500" />{s}</li>)}</ul>
          </div>
        </div>
      </div>
      ...
      {report.coachingSummary && (
        <div className="rounded-3xl bg-white p-6 shadow-card">
          <h3 className="flex items-center gap-2 text-title font-semibold"><Bot size={18} className="text-brand-500" /> AI Coach</h3>
          <p className="mt-2 text-body text-neutral-600">{report.coachingSummary}</p>
        </div>
      )}

      {report.practicePlan?.length > 0 && (
        <div className="rounded-3xl bg-white p-6 shadow-card">
          <h3 className="text-title font-semibold">7-Day Practice Plan</h3>
          <ol className="mt-3 space-y-1.5">
            {report.practicePlan.map((p, i) => <li key={i} className="text-body text-neutral-700"><span className="font-mono text-micro text-brand-600">{String(i + 1).padStart(2, '0')}</span> · {p}</li>)}
          </ol>
        </div>
      )}
      ...
      {fullReview && (
        <div className="rounded-3xl bg-white p-6 shadow-card">
          ...
          <div className="mt-3 space-y-3">
            {fullReview.questions.filter((q) => q.answer).map((q, i) => (
              <details key={i} className="rounded-2xl border border-neutral-200 p-4">
                <summary className="flex cursor-pointer items-center justify-between gap-3 text-body font-semibold">
                  ...
                  <span className="font-mono text-caption text-brand-600">{q.evaluation?.overall}/100</span>
                </summary>
                <p className="mt-2 text-caption text-neutral-500"><span className="font-bold">Your answer:</span> {q.answer}</p>
                {q.wentWell?.length > 0 && <p className="mt-2 text-caption text-emerald-700">✓ {q.wentWell.join(' · ')}</p>}
                {q.improve?.length > 0 && <p className="mt-1 text-caption text-amber-700">→ {q.improve.join(' · ')}</p>}
                {q.recommendedAnswer && <p className="mt-2 text-caption text-neutral-600"><span className="font-bold">Suggested:</span> {q.recommendedAnswer}</p>}
              </details>
            ))}
          </div>
        </div>
      ...
      <div className="flex flex-wrap justify-center gap-3 pb-8">
        ...
        <button onClick={() => { setConfig({ ...config, focus: (report.weaknesses || []).slice(0, 2).join(', ') }); setMode('setup'); }}
          className="press rounded-xl border border-neutral-200 bg-white px-6 py-3 text-body font-semibold text-neutral-700 shadow-soft">Practice Weak Areas</button>
      </div>
    </div>
  );

  // ═══════════════════ LIVE INTERVIEW (full-screen, dark) ═══════════════════
  return (
    <div className="fixed inset-0 z-50 flex flex-col overflow-y-auto bg-neutral-950 text-white">
      ...
      <div className="flex items-center justify-between px-4 sm:px-8 py-4 border-b border-neutral-800">
        ...
        <div className="flex items-center gap-4 font-mono text-caption text-neutral-400">
          <span>Question {qIndex}/{config.questionCount}</span>
      ...
      <div className="flex flex-1 flex-col items-center justify-center gap-5 px-6 py-8 text-center">
        ...
        <AnimatePresence mode="wait">
          {current && (
            <motion.div key={current.question} initial={{ opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -14 }}
              ...
              <span className="text-micro font-bold uppercase tracking-wider text-brand-400">{current.questionType} · L{current.difficulty}</span>
        ...
        {(liveStage === 'listening' || transcript) && (
          <div className="w-full max-w-2xl rounded-2xl border border-rose-500/30 bg-rose-500/5 p-4 text-left">
            <p className="text-micro font-bold uppercase tracking-wider text-rose-400">Live transcript</p>
            <p className="mt-1 max-h-28 overflow-y-auto text-body text-neutral-200">{transcript || '…'}</p>
          </div>
        )}

        {typedMode && liveStage === 'waiting' && (
          <textarea value={typedAnswer} onChange={(e) => setTypedAnswer(e.target.value)} rows={3}
            placeholder="Type your answer…"
            className="w-full max-w-2xl rounded-2xl border border-neutral-700 bg-neutral-900 p-4 text-body focus:border-brand-500 focus:outline-none" />
        )}
      </div>
      ...
      <div className="flex items-center justify-center gap-4 sm:gap-6 border-t border-neutral-800 px-6 py-5">
        ...
        {typedMode ? (
          <button onClick={() => submitAnswer(typedAnswer)} disabled={liveStage === 'processing'}
            className="press flex h-16 w-16 items-center justify-center rounded-full bg-gradient-brand text-white shadow-glow disabled:opacity-50">
